import { chatRoom} from "src/app/api/chatRoom/chatRoom.api";
...
export const getChatRoomData = async (nickname:string) => {
  try {
    const chatRooms = await chatRoom.findAllByNickname(nickname); // API 호출
...
export const deleteChatRoomsService = async (chatRoomIds: string[]) => {
  ...
  await Promise.all(chatRoomIds.map(id => chatRoom.deleteById(id)));
};
